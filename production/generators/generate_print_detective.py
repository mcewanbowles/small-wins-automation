...
LEVEL_COLORS = {1: hex_to_rgb("#5BBE72"), 2: hex_to_rgb("#4AA3DF"), 3: hex_to_rgb("#F2B441"), 4: hex_to_rgb("#EF6A62")}

# Letters vs non-letters (L1) and letters vs words (L2)
L1_LETTERS = ["A", "L", "M", "P", "S", "T"]
L1_NON = ["3", "7", "?", "!", "&", "@"]
L2_LETTERS = ["b", "m", "s", "h", "w", "l"]
L2_WORDS = ["llama", "mama", "bed", "hug", "night", "cry"]

# L3 first-sound pairs: letter -> (icon file stem, word label)
L3_PAIRS = [
    ("L", "llama_llama", "llama"),
    ("P", "pajamas", "pajamas"),
    ("B", "bed", "bed"),
...
L4_SENTENCE = ["Mama", "hugs", "little", "llama"]
...
def _answer_key(page_num: int, total: int) -> Image.Image:
    ...
    rows = [
        ...
        ("Level 3", "L→llama  P→pajamas  B→bed  M→mama  N→night  C→cry  S→stairs"),
        ("Level 4", "Sentence has 4 words: Mama / hugs / little / llama. First = Mama, last = llama."),
    ]
    y = 700
...
def _intro(page_num: int, total: int) -> Image.Image:
    ...
    items = [
        ...
        "",
        "All pictures and words come from your Llama Llama",
        "Red Pajama vocabulary - the same icons students",
...
def generate_print_detective_pack() -> dict:
    ...
    lines = [
        ...
        ("LINKS TO SUITE", "All pictures and words reuse the reviewed LLRP vocabulary and icons, so the activity reinforces the same word set as Matching, Sorting and CVC Decode & Build."),

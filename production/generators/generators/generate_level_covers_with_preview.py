...
def create_level_cover_with_preview(level, theme_name, product_type, level_pdf_path, output_path, pack_code="SWS-MTCH-001"):
    ...
    # {PACK_CODE} | {THEME} | Level {X} | Page {Y}/{TOTAL} @ 2026 Small Wins Studio. PCS® symbols used with active PCS Maker Personal License.
    
    c.setFillColor(LIGHT_GRAY)
    c.setFont(regular_font, 9)
    
    # Footer text - Page 1 of total
    footer_text = f"{pack_code} | {theme_name} | Level {level} | Page 1/{total_pages} © 2026 Small Wins Studio. PCS® symbols used with active PCS Maker Personal License."

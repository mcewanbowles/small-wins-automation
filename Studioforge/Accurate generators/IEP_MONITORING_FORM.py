...
def _font(pt, bold=False):
    ...
    pop_b = assets / "Poppins-Bold.ttf"
    pop_m = assets / "Poppins-Medium.ttf"
    pop_r = assets / "Poppins-Regular.ttf"
    nun_r = assets / "Nunito-Regular.ttf"
    nun_b = assets / "Nunito-Bold.ttf"
...
def _header(page, draw, book_title, pack_code, scale):
    ...
    # Content starts below the accent strip (which is ~1.0" tall by default)
    # Add extra gap to ensure form fields don't collide with the subtitle text
    y = int(1.15 * DPI)
    margin = int(28 * scale)
    lf = _font(7.5)
...
def generate_iep_monitoring_form(
    ...
    apply_small_wins_frame(
        ...
        subtitle=f"{book_title} — Small Wins Studio",
        pack_code=pack_code,
        page_num=1,
        total_pages=1,
        level=None,
    ...
    # Subtle right-panel tint (within border, below accent strip)
    border_inset = int(26 * scale)
    accent_bottom = int(1.0 * DPI)
    draw.rectangle([int(PW * 0.62), accent_bottom, PW - border_inset, PH - border_inset], fill=(250, 253, 252))

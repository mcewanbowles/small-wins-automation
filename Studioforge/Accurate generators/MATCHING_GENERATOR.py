...
def create_matching_page(loaded_images, image_names, target_index, level, page_num, pack_code, theme_name, qa_warnings=None, header_icon_img=None):
    ...
    # Reduce header clearance further to move activity higher (hero lives in header)
    header_clear = int(1.10 * DPI)
    ...
    draw.text((instruction_x, content_top + int(0.10 * DPI)), instruction_text, fill=hex_to_rgb(NAVY_BLUE), font=fonts['subtitle'])
    ...
    gap_x = int(0.90 * DPI)  # wider gap between LEFT/RIGHT columns for clearer separation
    gap_y = int(0.12 * DPI)
    box_size = min(
    ...
    for i in range(5):
        ...
        # Velcro circle (no text label inside to keep card clean)
...
def create_cutouts_page(loaded_images, image_names, page_num, pack_code, theme_name, header_icon_img=None):
    ...
    # Tip only (remove 'Cut along lines' sentence)
    tip_y = y_bot - int(0.70 * DPI)
...
def create_storage_labels_page(loaded_images, image_names, levels, page_num, pack_code, theme_name, header_icon_img=None):
    ...
    start_y = header_clear + int(0.24 * DPI)
...
def generate_matching_pack(images_folder, pack_code="WAN1-A", theme_name="Winter Animals"):
    ...
    cutouts = create_cutouts_page(loaded_images, image_names, page_num, pack_code, theme_name, header_icon_img=header_icon_img)
    saved_pages.append(cutouts)
    page_num += 1
    
    # Storage Labels
    print(f"\nðŸ“„ Storage Labels (Pages {page_num}-{page_num+1})...")
    labels_1_2 = create_storage_labels_page(loaded_images, image_names, "1-2", page_num, pack_code, theme_name, header_icon_img=header_icon_img)
    saved_pages.append(labels_1_2)
    page_num += 1
    
    labels_3_4 = create_storage_labels_page(loaded_images, image_names, "3-4", page_num, pack_code, theme_name, header_icon_img=header_icon_img)

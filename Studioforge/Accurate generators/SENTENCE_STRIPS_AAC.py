...
def create_sentence_strip_page(sentence_pattern, aac_symbols_needed, theme_images, 
    ...
    for strip_idx in range(strips_per_page):
        ...
        # This pattern supports: AAC + AAC + IMAGE or AAC + IMAGE + AAC
        ...
        # Start position for centered strip
        n_boxes = len(sentence_pattern['components'])
        total_width = n_boxes * box_size + (n_boxes - 1) * box_spacing
        start_x = margin_left + (content_width - total_width) // 2
        
        current_x = start_x
        
        # Place AAC symbols and theme image according to pattern
        for component in sentence_pattern['components']:
            box_y = strip_y + (strip_height - box_size - label_space) // 2
            
            # Draw box
            draw.rounded_rectangle(
                [current_x, box_y, current_x + box_size, box_y + box_size],
                radius=int(6 * scale),
                fill='white',
            ...
            if component['type'] == 'aac':
                ...
                if aac_key in aac_symbols:
                    ...
                    img_x = current_x + (box_size - img_copy.width) // 2
                ...
                if show_aac_labels:
                    ...
                    usable_nm_w = box_size - int(10 * scale)
                    ...
                    name_x = current_x + (box_size - name_w) // 2
                ...
                img_x = current_x + (box_size - theme_copy.width) // 2
            ...
            current_x += box_size + box_spacing
        ...
        if show_cut_guides and strip_idx < strips_per_page - 1:
            ...
            try:
                draw.text((margin_left - int(18 * scale), y_mid - int(8 * scale)), "✂", fill=(150,150,150), font=fonts['instruction'])
            except Exception:
                pass

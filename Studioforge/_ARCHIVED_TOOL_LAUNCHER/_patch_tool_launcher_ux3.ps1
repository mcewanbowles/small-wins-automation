...
# 1) Header Diagnostics button after the title label
...
# 2) Remember last-selected book slug after books list
...
if ($content -notmatch 'def\s+_init_board_tab[\s\S]*Open Output Folder') {
...
$repBRok = '${1}self._remember_last_book(slug)`r`n            _touch_used("board_ready")'
$new = [regex]::Replace($content,$patternBRok,$repBRok,1)
if ($new -ne $content) { $content = $new; $changes += 'remember_on_launch_br' }

$patternMTok = '(?ms)(def\s+_launch_matching[\s\S]*?if ok:\r?\n\s{12})_touch_used\("matching"\)'
$repMTok = '${1}self._remember_last_book(slug)`r`n            _touch_used("matching")'
$new = [regex]::Replace($content,$patternMTok,$repMTok,1)
if ($new -ne $content) { $content = $new; $changes += 'remember_on_launch_mt' }

$patternLTok = '(?ms)(def\s+_launch_listing[\s\S]*?if ok:\r?\n\s{12})_touch_used\("listing_generator"\)'
$repLTok = '${1}self._remember_last_book(slug)`r`n            _touch_used("listing_generator")'
$new = [regex]::Replace($content,$patternLTok,$repLTok,1)
if ($new -ne $content) { $content = $new; $changes += 'remember_on_launch_lt' }

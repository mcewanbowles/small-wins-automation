...
from utils.sws_design import apply_small_wins_frame, row_gap_px, safe_footer_inset_px, generate_internal_cover_page, generate_teacher_cover_page, _brand_font_pt, generate_storage_label_page
...
def _page_to_pdf(pages: list[Image.Image], pdf_path: Path, *, watermark_preview: bool) -> None:
    ...
    for p in pages:
        # Detect per-page orientation from image dimensions
...
def generate_story_strips(
    ...
    total_pages = 3  # 2 activity pages + 1 storage label
    scale = DPI / 72
    ...
    # Storage label page
    storage = generate_storage_label_page(
        product_title="Sequencing",
        theme_name=theme_name,
        pack_code=pack_code,
        page_num=3,
        total_pages=total_pages,
        labels=[{"header": "Sequencing Cards", "subtitle": "Story Picture Cards"}],
        footer_title=f"{theme_name} | Sequencing",
    )

...
try:
    ...
    pass
from utils.sws_design import generate_internal_cover_page, generate_teacher_cover_page, shrink_font_to_fit_with_pt, normalize_text, apply_small_wins_frame, generate_storage_label_page
...
def _review_page(
    ...
    *,
    ...
    box = [int(0.33 * DPI), int(1.30 * DPI), PAGE_W - int(0.33 * DPI), int(2.15 * DPI)]
    ...
    d.text((int(0.40 * DPI), int(2.50 * DPI)), "Words from this story:", fill=hex_to_rgb(TITLE_BLUE), font=fonts["page_header"])
    # grid 4x2
    cols, rows = 4, 2
...
def _cutout_page(slug: str, words: list[str]) -> Image.Image:
    ...
    d.text((PAGE_W // 2 - int(1.8 * DPI), int(1.30 * DPI)), "Print - Laminate - Cut", fill=hex_to_rgb(STEEL_BLUE), font=fonts["instruction"])
    cols, rows = 4, 2
...
def _storage_page(slug: str, title: str, pack_code: str, words: list[str]) -> Image.Image:
    """Use the shared branded storage label page (consistent across all products)."""
    return generate_storage_label_page(
        product_title="Adapted Book",
        theme_name=title,
        pack_code=pack_code,
        page_num=0,
        total_pages=0,
        labels=[{"header": "Adapted Book", "subtitle": title}],
    )

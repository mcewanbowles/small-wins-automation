...
try:
    from utils.sws_design import apply_small_wins_frame, _brand_font_pt, generate_teacher_cover_page, generate_storage_label_page
except Exception:
    try:
...
def _make_snap_section_divider(book_title: str, pack_code: str, variant: str, page_num: int, total_pages: int, color: bool, header_icon: Image.Image | None = None) -> Image.Image:
    """Section divider page for the consolidated WordSnap PDF."""
    page = Image.new("RGB", (SNAP_W, SNAP_H), PAGE_CREAM)
    draw = ImageDraw.Draw(page)
    scale = DPI / 72
    variant_info = {
        "symbol_text": {
            "title": "Symbol + Text Cards",
            "subtitle": "Matching & Memory Game",
            "description": "Each word appears twice — symbol with text label.\nPlay Snap, Memory, or Go Fish.\nLaminate for repeated classroom use.",
        },
        "symbol_only": {
            "title": "Symbol Only Cards",
            "subtitle": "Receptive Identification",
            "description": "Symbol only — no text.\nUse for receptive identification:\n'Show me the backpack.'\nIdeal for non-verbal students.",
        },
        "text_only": {
            "title": "Text Only Cards",
            "subtitle": "Sight-Word Recognition",
            "description": "Text only — no symbol.\nUse for sight-word recognition\nand symbol-to-word matching.\nPair with symbol cards for matching.",
        },
        "extra_copy": {
            "title": "Extra Copy Cards",
            "subtitle": "Additional Symbol + Text Set",
            "description": "Extra copies of the symbol + text deck.\nUse for matching, sorting,\nor send home for practice.\nSame words as the main deck.",
        },
    }
    info = variant_info.get(variant, variant_info["symbol_text"])
    apply_small_wins_frame(
        page,
        product_title=book_title,
        subtitle=f"Vocabulary Snap — {info['title']}",
        pack_code=pack_code,
        page_num=page_num,
        total_pages=total_pages,
        level=None,
    ...
    center_x = SNAP_W // 2
    title_font = _brand_font_pt(36, bold=True, brand="poppins")
    sub_font = _brand_font_pt(22, bold=False, brand="poppins")
    desc_font = _brand_font_pt(16, bold=False, brand="poppins")
    navy = hex_to_rgb(SWS_NAVY) if color else (0, 0, 0)
    teal = hex_to_rgb(SWS_TEAL) if color else (100, 100, 100)
    # Title
    title_y = int(2.5 * DPI)
    tb = draw.textbbox((0, 0), info["title"], font=title_font)
    tw = tb[2] - tb[0]
    draw.text((center_x - tw // 2, title_y), info["title"], fill=navy, font=title_font)
    # Subtitle
    sub_y = title_y + int(0.55 * DPI)
    sb = draw.textbbox((0, 0), info["subtitle"], font=sub_font)
    sw = sb[2] - sb[0]
    draw.text((center_x - sw // 2, sub_y), info["subtitle"], fill=teal, font=sub_font)
    # Divider line
    line_y = sub_y + int(0.45 * DPI)
    line_w = int(3.0 * DPI)
    draw.rounded_rectangle([center_x - line_w // 2, line_y, center_x + line_w // 2, line_y + int(3 * scale)], radius=int(2 * scale), fill=teal)
    # Description
    desc_y = line_y + int(0.35 * DPI)
    for line in info["description"].split("\n"):
        db = draw.textbbox((0, 0), line, font=desc_font)
        dw = db[2] - db[0]
        draw.text((center_x - dw // 2, desc_y), line, fill=navy, font=desc_font)
        desc_y += int(0.30 * DPI)
    return page
...
def generate_word_wall(slug: str, book_title: str,
    ...
    OUTPUT:
      ...
      {pack_code}_WordSnap_COLOR.pdf — consolidated deck with all 4 variants + section dividers
      {pack_code}_WordSnap_BW.pdf — same, black & white
      {pack_code}_WordSnap_PREVIEW.pdf — 3-page preview (divider + first 2 card pages)
    ...
    # Ensure book title is title-cased (fixes lowercase slug leaking through)
    if book_title and book_title == book_title.lower():
    ...
    # Shuffle to avoid identical cards appearing adjacently on the same page
    if len(vocab_cards) > CARDS_PER_PAGE:
        rng = random.Random(42)
        rng.shuffle(vocab_cards)
    ...
    try:
        wall_thumb_dir = output_dir / "thumbnails"
        wall_thumb_dir.mkdir(parents=True, exist_ok=True)
        wall_thumb_paths = []
        for index, page in enumerate(pages_color[:2], start=1):
            thumb = page.copy()
            thumb.thumbnail((500, 647), Image.Resampling.LANCZOS)
            thumb_path = wall_thumb_dir / f"{pack_code}_WordWall_thumb{index}.png"
            thumb.save(thumb_path, "PNG")
            wall_thumb_paths.append(str(thumb_path))
    except Exception:
        wall_thumb_paths = []
    ...
    # Banner thumbnails
    try:
        banner_thumb_dir = output_dir / "thumbnails"
        banner_thumb_dir.mkdir(parents=True, exist_ok=True)
        for index, page in enumerate(pages_banner[:2], start=1):
            thumb = page.copy()
            thumb.thumbnail((500, 647), Image.Resampling.LANCZOS)
            thumb_path = banner_thumb_dir / f"{pack_code}_WordWall_Banner_thumb{index}.png"
            thumb.save(thumb_path, "PNG")
    except Exception:
        pass
    ...
    snap_thumbnails = []
    variant_thumb_map = {
        "symbol_text": "WordSnap_SymbolText",
        "symbol_only": "WordSnap_SymbolOnly",
        "text_only": "WordSnap_TextOnly",
        "extra_copy": "WordSnap_ExtraCopy",
    }
    # ── Consolidated WordSnap PDF ──────────────────────────────────────────────
    # All 4 variants merged into ONE color PDF and ONE BW PDF with section dividers.
    # Individual variant PDFs are no longer written — teachers get a single file.
    deck_order = ("symbol_text", "symbol_only", "text_only", "extra_copy")
    # Pre-build all variant pages and count total for page numbering
    variant_pages_color: dict[str, list[Image.Image]] = {}
    variant_pages_bw: dict[str, list[Image.Image]] = {}
    variant_missing: dict[str, list[str]] = {}
    for deck_key in deck_order:
        pc, mc = build_snap_pages(True, deck_key)
        pb, mb = build_snap_pages(False, deck_key)
        variant_pages_color[deck_key] = pc
        variant_pages_bw[deck_key] = pb
        variant_missing[deck_key] = mc
        snap_manifest["variants"][f"{deck_key}_color"] = {
            "cards": len(snap_decks[deck_key]),
            "pages": len(pc),
        }
        snap_manifest["variants"][f"{deck_key}_bw"] = {
            "cards": len(snap_decks[deck_key]),
            "pages": len(pb),
        }
        snap_manifest["missing_icons"].extend(mc)
    # Total pages = sum of (1 divider + N card pages) per variant
    total_snap_pages = sum(1 + len(variant_pages_color[d]) for d in deck_order) + 1  # +1 storage label
    # Build consolidated page lists with dividers
    consolidated_color: list[Image.Image] = []
    consolidated_bw: list[Image.Image] = []
    page_idx = 1
    for deck_key in deck_order:
        # Divider page
        consolidated_color.append(_make_snap_section_divider(
            book_title, pack_code, deck_key, page_idx, total_snap_pages, True, header_icon_img))
        consolidated_bw.append(_make_snap_section_divider(
            book_title, pack_code, deck_key, page_idx, total_snap_pages, False, header_icon_img))
        page_idx += 1
        # Card pages
        for p in variant_pages_color[deck_key]:
            consolidated_color.append(p)
        for p in variant_pages_bw[deck_key]:
            consolidated_bw.append(p)
        page_idx += len(variant_pages_color[deck_key])
    # Storage label page
    storage_label = generate_storage_label_page(
        product_title="Vocabulary Snap",
        theme_name=book_title,
        pack_code=pack_code,
        page_num=page_idx,
        total_pages=total_snap_pages,
        labels=[{"header": "Vocabulary Snap Cards", "subtitle": "Symbol+Text, Symbol-Only, Text-Only"}],
        footer_title=f"{book_title} | Vocabulary Snap",
    )
    consolidated_color.append(storage_label)
    consolidated_bw.append(storage_label.convert("L").convert("RGB"))
    # Write consolidated PDFs
    consolidated_color_path = output_dir / f"{pack_code}_WordSnap_COLOR.pdf"
    consolidated_bw_path = output_dir / f"{pack_code}_WordSnap_BW.pdf"
    to_pdf_mixed(consolidated_color, consolidated_color_path)
    to_pdf_mixed(consolidated_bw, consolidated_bw_path)
    print(f"  ✓  {consolidated_color_path} ({len(consolidated_color)} pages)")
    print(f"  ✓  {consolidated_bw_path} ({len(consolidated_bw)} pages)")
    # Preview PDF — first 3 pages of symbol_text section (divider + 2 card pages)
    preview_pages = consolidated_color[:min(3, len(consolidated_color))]
    to_pdf_mixed(preview_pages, snap_preview_path, preview=True)
    print(f"  ✓  {snap_preview_path}")
    # Thumbnails from symbol_text section
    try:
        thumb_dir = output_dir / "thumbnails"
        thumb_dir.mkdir(parents=True, exist_ok=True)
        snap_thumbnails = []
        for index, page in enumerate(variant_pages_color.get("symbol_text", [])[:2], start=1):
            thumb = page.copy()
            thumb.thumbnail((500, 647), Image.Resampling.LANCZOS)
        ...
        pass
    # Clean up old individual variant PDFs if they exist
    for deck_key in deck_order:
        for color in (True, False):
            old_path = snap_paths.get((deck_key, color))
            if old_path and old_path.exists():
                try:
                    ...
                    pass
    snap_manifest["files"] = {
        "color_pdf": str(consolidated_color_path),
        "bw_pdf": str(consolidated_bw_path),
        "preview_pdf": str(snap_preview_path),
        "thumbnails": snap_thumbnails,
    }

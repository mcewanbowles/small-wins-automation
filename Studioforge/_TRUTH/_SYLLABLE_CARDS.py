...
from utils.sws_design import apply_small_wins_frame, shrink_font_to_fit_with_pt, hex_to_rgb, NAVY_HEX, DPI, safe_footer_inset_px, generate_storage_label_page
...
def _draw_syllable_cards_page(*, theme_name: str, pack_code: str, page_num: int, total_pages: int, index: int, items: List[Tuple[str, Image.Image, int, str]], header_left_icon: Image.Image | None) -> Image.Image:
    ...
    for r in range(2):
        for c in range(2):
            idx = r * 2 + c
            if idx >= len(items):
                continue  # Skip blank cells — don't draw empty boxes
            bx0 = x0 + c * (box_w + gap)
            by0 = y0 + r * (box_h + gap)
            # Box border
...
def generate_syllable_cards_pack(images_folder: str, pack_code: str = "SYL01", theme_name: str = "Theme") -> bool:
    ...
    total_pages = page_count + 1  # +1 storage label
    ...
    storage_label = generate_storage_label_page(
        product_title="Syllable Awareness",
        theme_name=theme_name,
        pack_code=pack_code,
        page_num=page_count + 1,
        total_pages=total_pages,
        labels=[{"header": "Syllable Cards", "subtitle": "Segmented Word Cards"}],
        footer_title=f"{theme_name} | Syllable Awareness",
    )
    pages_color.append(storage_label)

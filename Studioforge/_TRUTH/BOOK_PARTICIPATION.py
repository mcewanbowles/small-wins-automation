...
def _answer_mat(theme_name: str, pack_code: str, labels: list[str], page_num: int, total_pages: int) -> Image.Image:
    ...
    for index, label in enumerate(labels[:12]):
        ...
        marker = (x + 45, y + 65, x + 165, y + 185)
        draw.ellipse(marker, fill=hex_to_rgb(WHITE), outline=hex_to_rgb(SWS_GOLD), width=4)
        _center(draw, (x + 195, y + 50, x + cell_w - 40, y + 205), label, _fit(label, 15, cell_w - 300, 10), hex_to_rgb(SWS_NAVY))
        _center(draw, (x + 80, y + 245, x + cell_w - 80, y + 420), "PLACE PIECE HERE", _font(10, True), hex_to_rgb(TEXT))
    return page
...
def generate_book_participation_pack(images_folder: str, pack_code: str = "BPP", theme_name: str = "Theme") -> bool:
    ...
    pages.append(_vocab_page(theme_name, pack_code, vocab[8:12], 4, total_pages))
    questions = list(data.get("question_strips") or [])
    pages.append(_question_page(theme_name, pack_code, questions[:2], 5, total_pages))
    pages.append(_question_page(theme_name, pack_code, questions[2:4], 6, total_pages))
    pages.append(_early_access_page(theme_name, pack_code, early_access, hero, turn_page, 7, total_pages))
    pages.append(_participation_page(theme_name, pack_code, participation, 8, total_pages))
    pages.append(_answer_mat(theme_name, pack_code, [label for label, _image in vocab], 9, total_pages))
    ...
    for index, page in enumerate((pages[1], pages[3], pages[6], pages[7], pages[8]), start=1):

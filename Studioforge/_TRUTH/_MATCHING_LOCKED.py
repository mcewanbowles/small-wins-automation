...
def create_matching_page(loaded_images, image_names, target_index, level, page_num, pack_code, theme_name, qa_warnings=None, header_icon_img=None):
    ...
    # Level configuration
    level_config = {
        1: {"targets": 5, "distractors": 0, "name": "Supported"},
        2: {"targets": 4, "distractors": 1, "name": "Developing"},
        3: {"targets": 3, "distractors": 2, "name": "Independent"},
        4: {"targets": 1, "distractors": 4, "name": "Extended"}
    }
    ...
    # Universal frame — consistent header/footer with level pill
    apply_small_wins_frame(
        ...
        pack_code=pack_code,
        page_num=page_num,
        total_pages=len(loaded_images) * 4 + 3,  # four levels plus cutouts and two label pages
        level=level,
    ...
    # Instruction at top of content (compact) — level-appropriate
    # Level 1 (errorless): all rows = target, student matches all
    # Level 2-3 (discrimination): some rows are target, some are distractors.
    #   Student places target pieces ONLY on matching rows — distractors test discrimination.
    # Level 4 (find one): 1 target among 4 distractors — pure visual search
    if num_distractors == 0:
        instruction_text = f"Match all to: {target_name}"
    elif num_targets == 1:
        instruction_text = f"Find the {target_name}"
    else:
        instruction_text = f"Match only the {target_name}"
...
def create_cutouts_page(loaded_images, image_names, page_num, pack_code, theme_name):
    ...
    apply_small_wins_frame(
        ...
        pack_code=pack_code,
        page_num=page_num,
        total_pages=len(loaded_images) * 4 + 3,
        level=None,
        footer_title=f"{theme_name} | Matching Cards",
        show_data_strip=False,
    )
    ...
    y_bot = img_height - safe_footer_inset_px() - int(0.78 * DPI)
...
def generate_matching_pack(images_folder, pack_code="WAN1-A", theme_name="Winter Animals", selected_stems=None):
    ...
    if selected_stems:
        available = {path.stem.lower(): path for path in image_files}
        missing = [stem for stem in selected_stems if str(stem).lower() not in available]
        if missing:
            print(f"Missing approved Matching icons: {', '.join(missing)}")
            return False
        image_files = [available[str(stem).lower()] for stem in selected_stems]
    ...
    for level in range(1, 5):
        level_names = {1: "Supported", 2: "Developing", 3: "Independent", 4: "Extended"}
    ...
    try:
        ...
        if design_mod is not None:
            ...
            if callable(gen_cov):
                whats = [
                    "4 differentiated levels (Supported to Extended)",
                ...
                tips = [
                    ...
                    "Start supported, then fade prompts across levels.",

...
def generate_find_and_cover_pack(images_folder, pack_code="WAN1-A", theme_name="Winter Animals", output_dir: str | None = None):
    ...
    try:
        theme_dir = images_path.resolve().parent
        hero_candidates = [
            # Prefer the reviewed activity_images icon (clean PCS symbol)
            theme_dir / "activity_images" / "llama_llama.png",
            # Fallback to characters dir (may contain older artwork)
        ...
        hero_path_str = None
        book_cover_path_str = None
    # Per QA: do NOT use published book cover artwork on Find & Cover cover.
    # Use the Small Wins Studio logo circle instead (same as AAC Board / Yes/No covers).
    book_cover_path_str = None
